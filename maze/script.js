...
function create(width = 3, height = 3, cellSize = 1) {
	...
	creatingMaze = true;
	let count = 0;
	for (let i = 0; i < h; i += parseInt(cs)) {
		for (let j = 0; j < w; j += parseInt(cs)) {
			store.push(new Cells(count, j, i, parseInt(cs)));
			count++;
		}
	}
	store[0].walls[3] = 0;
	...
	let maxW = width / cellSize - 1, n = width / cellSize * height / cellSize;
